...
factor	: variable  // factor can be void
	...
	| variable INCOP 
	{
		...
		VarNode* lhsVar = (VarNode*)$1->get_ast_node();
		// Create a separate VarNode for the RHS to avoid double-free
		VarNode* rhsVar = new VarNode(lhsVar->get_name(), lhsVar->get_type(), 
		                             nullptr); // Simplified for now
